...
%MSBuildPath%\MSBuild.exe "%SolutionOrProjectPath%" /t:rebuild /p:configuration="Debug" >>"%~dp0%BUILD_MKDIR%\%LOG_MKDIR%\Debug_%FULLDATE%_%FULLTIME%.txt"

FOR  %%i in ("%~dp0Debug\*.exe") DO (SET PROGRAM_NEME=%%~ni)

set PROGRAM_DIR=%PROGRAM_NEME%_%PROGRAM_VER%" "(Build" "%BUILDCODE%)
MKDIR %PROGRAM_DIR%
cd "%~dp0"

copy "%~dp0Debug\*.exe" %BUILD_MKDIR%\%PROGRAM_DIR%
